...
// Метаданные статусов — единый стиль как в STATUS_META из стейджей
const COL: Record<TaskStatus, {
  icon: React.ComponentType<{ className?: string }>;
  textColor: string;     // цвет текста заголовка + иконки
  dot: string;           // цвет точки-индикатора
  border: string;        // border карточки и dashed-зоны
  chip: string;          // chip-стиль для подписи статуса
  empty: string;
}> = {
  backlog: {
    icon: Circle,
...
function Ava({ name, url, size = 'sm' }: { name?: string | null; url?: string | null; size?: 'xs' | 'sm' | 'md' }) {
  const c = { xs: 'w-6 h-6 text-[13px]', sm: 'w-7 h-7 text-[11px]', md: 'w-9 h-9 text-base' }[size];
...
/* ═══════════════════════════════════════════════════════════════════
   TASK CARD — стиль карточки этапа из стейджей
   ═══════════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════════
   KANBAN COLUMN — стиль колонки из стейджей
   ═══════════════════════════════════════════════════════════════════ */
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-xl max-h-[90vh] flex flex-col bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl overflow-hidden"
          ...
          {canReview && (
            <div className="p-4 rounded-xl bg-[var(--hover-2)] border border-[var(--border-color)]">
              ...
              <div className="flex gap-2">
                <button onClick={() => act('review', () => tasksApi.review(task.id, { action: 'approve' }), 'Принято')}
                ...
                <button onClick={() => act('review', () => tasksApi.review(task.id, { action: 'reject' }), 'На доработку')}
                  disabled={busy === 'review'}
        ...
        <div className="flex items-center justify-between gap-3 px-5 py-3.5 border-t border-[var(--border-color)] bg-[var(--hover-1)] flex-shrink-0">
          <button onClick={() => act('archive', () => tasksApi.archive(task.id), 'Архивировано')}
            disabled={busy === 'archive'}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[var(--accent)]/10 hover:bg-[var(--accent)]/20 border border-[var(--accent)]/20 text-[var(--accent)] text-base font-medium disabled:opacity-50">
...
export default function TasksPage() {
  ...
  const staff = ['admin', 'support_manager', 'support_agent', 'executor'].includes(userRole);
  ...
  useEffect(() => {
    ...
    setMode(prev => (prev === 'project' || prev === 'assignee') ? prev : (staff ? 'internal' : 'my'));
  }, [up, ua, staff]);
  ...
  const loadP = useCallback(async () => {
    setLpj(true);
    try {
      setProjects((staff ? await projectsApi.getAll(1, 100, 'active') : await projectsApi.getMyProjects('all', 1, 100)).items);
    } catch { setProjects([]); }
                        ...
                        {TASK_PRIORITY_LIST.map(p => {
                          ...
                          return (
                            <button key={p} onClick={() => setFp(v => v.includes(p) ? v.filter(x => x !== p) : [...v, p])}
                              className={`flex items-center gap-1 px-2 py-1 rounded-lg text-base font-medium border transition-all
                                ${fp.includes(p) ? `${pm.bg} ${pm.color} ${pm.border}` : 'bg-[var(--hover-1)] text-[var(--text-primary)]/40 border-[var(--border-color)]'}`}>
                              <span className={`w-1.5 h-1.5 rounded-full ${pm.dot}`} />
                              {p}
                            </button>
                          );
