  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        `}
      >
        <span className={value ? 'text-red-400' : 'text-white/40'}>{icon}</span>
      ...
      {open && (
        <div
          className={`
            absolute z-[100] min-w-[200px] w-max bg-[#1e1e1e] border border-white/[0.12]
...
function ActiveFilters({
  ...
  return (
    <div className="flex items-center gap-2.5 flex-wrap">
      <span className="text-base text-white/40 flex items-center gap-1.5">
        <Filter size={14} /> Фильтры:
      </span>
      {category && catInfo && (
        <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg ${catInfo.bg} border border-white/[0.08] text-base ${catInfo.color}`}>
          <catInfo.icon size={14} />
          {catInfo.label}
          <span onClick={onClearCategory} className="ml-0.5 p-0.5 rounded hover:bg-white/10 text-white/30 hover:text-white/60 cursor-pointer transition-colors">
            <X size={12} />
          </span>
        </span>
      )}
      {status && sInfo && (
        <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/[0.06] border border-white/[0.08] text-base text-white/80">
          <span className={`w-2.5 h-2.5 rounded-full ${sInfo.dot}`} />
          {sInfo.label}
          <span onClick={onClearStatus} className="ml-0.5 p-0.5 rounded hover:bg-white/10 text-white/30 hover:text-white/60 cursor-pointer transition-colors">
            <X size={12} />
          </span>
        </span>
      )}
  ...
  return (
    ...
    >
      <div className={`flex-shrink-0 w-10 h-10 rounded-xl ${cat?.bg || 'bg-white/[0.06]'}
                      flex items-center justify-center ${cat?.color || 'text-white/40'}
      ...
      <div className="flex-1 min-w-0">
        ...
        {product.description && (
          <p className="text-base text-white/40 mt-0.5 leading-snug">
            {truncate(product.description)}
          </p>
        )}
      </div>
      <div className="flex items-center gap-5 flex-shrink-0">
        <span className="text-base text-white/50 hidden md:block w-[110px] truncate text-right">{product.vendor}</span>
        <span className="text-base text-white/30 hidden lg:block w-[70px] text-right font-mono">
          {product.version ? `v${product.version}` : '—'}
        </span>
        <span className={`text-base px-2.5 py-1 rounded-lg ${cat?.bg || 'bg-white/[0.06]'} ${cat?.color || 'text-white/50'} hidden sm:block w-[90px] text-center`}>
        ...
        <div className="flex items-center gap-2 w-[100px] justify-end">
          ...
          <span className={`text-base ${sInfo?.color || 'text-white/40'}`}>{statusLabel(product.status)}</span>
        </div>
      </div>
    </button>
  );
...
const ProductModal = ({ product, onClose }: { product: any; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-8">
      ...
      >
        <div className="flex items-center justify-between gap-4 px-6 py-5 border-b border-white/[0.08] flex-shrink-0 bg-white/[0.02]">
          <div className="flex items-center gap-4 min-w-0">
            <div className={`w-12 h-12 rounded-xl ${cat?.bg || 'bg-white/[0.06]'} flex items-center justify-center ${cat?.color || 'text-white/40'} flex-shrink-0`}>
              <Icon size={22} />
            </div>
            <div className="min-w-0">
              <h2 className="text-xl font-bold text-white truncate">{product.display_name || product.name}</h2>
              <p className="text-base text-white/50 truncate">{product.vendor}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2.5 rounded-xl hover:bg-white/[0.08] text-white/40 hover:text-white transition-colors flex-shrink-0">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-6 space-y-6">
          <div className="flex flex-wrap gap-2.5">
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.05] text-base text-white/60 border border-white/[0.08]">
              <Building2 size={16} className="text-white/40" />{product.vendor}
            </div>
            {product.version && (
              <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.05] text-base text-white/60 border border-white/[0.08] font-mono">
                <Tag size={16} className="text-white/40" />v{product.version}
              </div>
            )}
            <div className={`flex items-center gap-2 px-3 py-2 rounded-xl ${cat?.bg || 'bg-white/[0.05]'} text-base ${cat?.color || 'text-white/60'} border border-white/[0.08]`}>
              {cat && <cat.icon size={16} />}{cat?.label || product.category}
            </div>
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.05] text-base border border-white/[0.08]">
              <div className={`w-2.5 h-2.5 rounded-full ${statusDot(product.status)}`} />
              <span className={sInfo?.color || 'text-white/50'}>{statusLabel(product.status)}</span>
            </div>
          </div>

          {product.description && (
            <div>
              <div className="text-xs uppercase tracking-widest text-white/30 mb-3 font-semibold">Описание</div>
          ...
          {attrEntries.length > 0 && (
            <div>
              <div className="text-xs uppercase tracking-widest text-white/30 mb-3 font-semibold">Характеристики</div>
              <div className="rounded-xl border border-white/[0.08] divide-y divide-white/[0.06] bg-white/[0.02]">
                {attrEntries.map(([key, value]) => (
                  <div key={key} className="flex items-start gap-4 px-5 py-3.5">
                    <span className="text-base text-white/40 w-[150px] flex-shrink-0 pt-0.5">{getAttrLabel(key)}</span>
          ...
          {!product.description && attrEntries.length === 0 && (
            <div className="text-center py-12 text-white/30 text-base">Нет дополнительной информации</div>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-white text-3xl md:text-4xl font-bold tracking-tight">Продукты</h2>
          <p className="text-base text-white/40 mt-1">Справочник ПО и оборудования</p>
        </div>
        <div className="flex items-center gap-2.5">
          <button
            onClick={loadProducts}
            className="p-2.5 rounded-xl hover:bg-white/[0.06] text-white/40 hover:text-white/70 transition-colors"
            title="Обновить"
          >
            <RefreshCcw size={18} />
          </button>
      ...
      <div className="flex flex-wrap items-center gap-2.5">
        ...
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input
            ...
            className="w-full pl-10 pr-10 py-3 glass-card border border-white/[0.08]
      ...
      <div className="flex items-center gap-4 px-5 py-2.5 text-xs uppercase
                      tracking-wider text-white/30 border-b border-white/[0.08] font-semibold">
        <div className="w-10" />
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center py-24 text-white/30">
          <Package size={40} className="mb-4 text-white/15" />
      ...
      {/* ── Пагинация ────────────────────────────────────────────────────── */}
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-4 border-t border-white/[0.08]">
          <button
