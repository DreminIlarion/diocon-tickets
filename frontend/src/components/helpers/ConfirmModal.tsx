...
export const ConfirmModal = ({
    ...
    switch (type) {
      ...
      case 'warning':
        return {
          ...
          confirmButton: 'bg-yellow-800 hover:bg-yellow-700 text-[var(--text-primary)]',
          titleColor: 'text-[var(--text-primary)]'
        };
      default:
        return {
          icon: <AlertTriangle className="w-6 h-6 text-blue-400" />,
          confirmButton: 'bg-blue-800 hover:bg-blue-700 text-[var(--text-primary)]',
          titleColor: 'text-[var(--text-primary)]'
        };
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 p-4" onClick={onClose}>
      <div className="bg-[var(--bg-secondary)] rounded-2xl w-full max-w-md overflow-hidden border border-[var(--border-color)]" onClick={e => e.stopPropagation()}>
        {/* Заголовок */}
        <div className="flex justify-between items-center px-6 py-4 border-b border-[var(--border-color)]">
          ...
          <button onClick={onClose} className="text-[var(--text-primary)]/50 hover:text-[var(--text-primary)] transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {/* Содержание */}
        <div className="p-6">
          <p className="text-[var(--text-primary)]/70 text-base">{message}</p>
        </div>
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-[var(--border-color)]">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-xl bg-[var(--hover-1)] hover:bg-[var(--hover-1)] text-[var(--text-primary)] transition-colors text-sm"
          >
            {cancelText}
          </button>
          <button
